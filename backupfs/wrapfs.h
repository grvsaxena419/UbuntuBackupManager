...
/* custom definition for ubuntu sys restore */
# define SAVEDIR "sls-data"

/* time delay for next update of modification time*/
# define TIME_DELAY 10000 //msecs
...
struct wrapfs_file_info {
	...
	int backedup; // check if the file is already backed up
};
...
// Method declarations used as helpers
extern int get_hashed_path(char *path,char *buf,int buflen);
extern int copy_file(char* input_file,char* output_file);
extern int backup_file(struct path lower_path);
extern int getrename_path(struct path lowerpath, struct path *finalpath, char * mountpoint_path /* = mountpoint_path */);
extern int char_dev_buffer_insert(char msg[],int len);
extern int update_wrapfs_modif_tym(void);
extern void get_updated_modif_tym(unsigned long data);

extern __kernel_time_t modif_tym; /* Used for storing latest value of modification time
					   *  (would be updated periodically or using ioctl )*/
