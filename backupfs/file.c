			    ...
			    size_t count, loff_t *ppos)
{
	...
	struct file *lower_file;
	struct dentry *dentry = file->f_path.dentry;	
	struct wrapfs_file_info *wfi;
	struct path lower_path;
	
	lower_file = wrapfs_lower_file(file);
	lower_path=lower_file->f_path;
	
	printk(KERN_EMERG"wrapfs_write: the modif tym of file is %ld and modif_tym is %ld",lower_path.dentry->d_inode->i_mtime.tv_sec,modif_tym);
	
	if(lower_path.dentry->d_inode->i_mtime.tv_sec < modif_tym)
	{
		wfi=WRAPFS_F(file);
		if(wfi->backedup==0)
		{
			...
			err = backup_file(lower_path);
			if(err<0)
			{
				// report error we need a unified function for this
			}
			else
			{
				wfi->backedup=1;
		...
		{
			printk(KERN_EMERG"file has been already backed up :)");
		}
	}
...
static int wrapfs_open(struct inode *inode, struct file *file)
{
	...
	if(lower_path.dentry->d_inode->i_mtime.tv_sec<modif_tym)
	{
		//printk(KERN_EMERG"wrapfs: Got request to open %s with flags %u",lower_path.dentry->d_name.name,file->f_flags);	
		if(file->f_flags&O_TRUNC)
		{
			printk(KERN_EMERG"wrapfs: Got flag O_TRUNC");		
			// backup needed in this case too				
			wfi=WRAPFS_F(file);
			printk(KERN_EMERG"wrapfs: calling backupfile");		
			err=backup_file(lower_path);
			printk(KERN_EMERG"wrapfs: retrurnd frm backupfile");	

			if(err)
			{
				printk(KERN_EMERG"Error %d in copying file being trucated cant do anything now\n",err);
				err = 0;
			}
			else
			{
			...
			printk(KERN_EMERG"wrapfs: continuing with open....");
		}	
	}
