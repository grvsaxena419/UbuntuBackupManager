...
static int wrapfs_read_super(struct super_block *sb, void *raw_data, int silent)
{
	...
	printk(KERN_EMERG"wrapfs_read_super called");
	if (!dev_name) {
		printk(KERN_ERR
...
{	
	...
	printk(KERN_EMERG"wrapfs_mount called modif_tym=%ld",modif_tym);
	
	//get_updated_modif_tym(0);
...
static int __init init_wrapfs_fs(void)
{
	...
	printk(KERN_INFO "Inserted wrapfs successfully");		
...
out:
	...
	setup_timer( &update_timer, get_updated_modif_tym, 0 );
	
	err1 = mod_timer( &update_timer, jiffies + msecs_to_jiffies(TIME_DELAY) );
	if (err1)
	{
...
//MODULE_DESCRIPTION("Wrapfs " WRAPFS_VERSION " (http://wrapfs.filesystems.org/)");
MODULE_LICENSE("GPL");
